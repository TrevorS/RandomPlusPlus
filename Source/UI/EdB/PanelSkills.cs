    ...
    public class PanelSkills : PanelBase
    {
        ...
        protected override void DrawPanelContent()
        {
            ...
            if (Widgets.ButtonInvisible(RectButtonClearSkills, false))
            {
                ...
                PawnRandomizer.PawnFilter.ResetSkills();
            }

            int skillCount = PawnRandomizer.PawnFilter.Skills.Count;
            float rowHeight = 26;
            ...
            try
            {
                ...
                Rect rect;
                Text.Font = GameFont.Small;
                foreach (var skillFilter in PawnRandomizer.PawnFilter.Skills)
                {
            ...
            finally
            {
                ...
                string labelText = string.Format("RandomPlus.PanelSkills.PassionSlider".Translate(),
                    PawnRandomizer.PawnFilter.passionRange.min,
                    PawnRandomizer.PawnFilter.passionRange.max);
                Widgets.Label(totalPassionLabelRect, labelText);
                Widgets.IntRange(totalPassionRangeRect, 2042, ref PawnRandomizer.PawnFilter.passionRange,
                    0,
                    PawnFilter.PassionMaxDefault,
                    "", 2);

                string skillRangeLabelText = string.Format("RandomPlus.PanelSkills.SkillSlider".Translate(),
                    PawnRandomizer.PawnFilter.skillRange.min,
                    PawnRandomizer.PawnFilter.skillRange.max == PawnFilter.SkillMaxDefault ? "∞" : PawnRandomizer.PawnFilter.skillRange.max.ToString());
                Widgets.Label(totalSkillLabelRect, skillRangeLabelText);
                Widgets.IntRange(totalSkillRangeRect, 2043, ref PawnRandomizer.PawnFilter.skillRange,
                    0,
